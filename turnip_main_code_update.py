...
def summarize_stock(code, stocks, criteria, entry, trans_vol, filepath):
    ...
    if stock_summary["days_ave"] and stock_summary["profit_ave"] >= 3  : # 平均利益率のフィルタリング
...
def step01(cdlst, paths, parameters, lists, overwrite, newdir, dlonly, generateonly) :
    ...
    for code in cdlst:
        ...
        if not generateonly and not os.path.exists(csv_path) or overwrite :
...
#           csv_url = f'https://download.finance.yahoo.co.jp/common/history/{code}.T.csv'
        ...
        if not generateonly and sleep > 0 : time.sleep(sleep)
